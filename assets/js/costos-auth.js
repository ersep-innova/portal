() => {
  ...
  const config = window.ERSEP_COSTOS_AUTH_CONFIG;
  let activeKey = null;
  ...
  async function unlock(password) {
    try {
      ...
      const verifier = await decryptWithKey(
        { iv: config.verifierIv, ciphertext: config.verifierCiphertext }, candidateKey
      );
      if (verifier?.ok !== true || verifier?.scope !== "costos-tarifas") return false;
  ...
  async function decryptPayload(payload) {
    if (!activeKey) throw new Error("Primero debés validar el acceso a Costos y Tarifas.");
  ...
  function init() {
    if (document.body.dataset.authRequired !== "costos-tarifas") return;
    const gate = document.getElementById("auth_gate");
    ...
    form?.addEventListener("submit", async event => {
      ...
      if (valid) {
        ...
        document.dispatchEvent(new CustomEvent("ersep-costos-authenticated"));
      } else {
      ...
      button.textContent = "Ingresar a Costos y Tarifas";
    });

    document.querySelectorAll("[data-costos-lock]").forEach(el => el.addEventListener("click", lock));
  }

  window.ERSEP_COSTOS_AUTH = {
    decryptPayload,
