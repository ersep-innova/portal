...
def _ensure_bootstrap_roles(cur, user_id: int, email: str) -> bool:
    """
    Si el correo está incluido en BOOTSTRAP_ADMIN_EMAILS,
    garantiza que el usuario tenga siempre AGENTE + ADMIN.

    Devuelve True si corresponde al bootstrap.
    """
...
def _bootstrap_admin(cur, email: str, sub: str, claims: dict):
    ...
    cur.execute("""
        ...
        ON CONFLICT (email) DO UPDATE SET
            ...
            nombre = COALESCE(
                NULLIF(usuarios.nombre, ''),
                EXCLUDED.nombre
            ),
            apellido = COALESCE(
                NULLIF(usuarios.apellido, ''),
                EXCLUDED.apellido
            ),
            updated_at = NOW()
    ...
    with connection() as conn:
        with conn.cursor() as cur:
            ...
            if not user:
                ...
                if not user:
                    raise HTTPException(
                        status_code=403,
                        detail=(
                            "Tu cuenta Google no está "
                            "habilitada para este sistema."
                        )
                    )
            ...
            # ==========================================
                ...
                # Mantener Google SUB actualizado
                ...
                # ======================================
                # RECUPERACIÓN AUTOMÁTICA DE ADMIN
                # ======================================

                if _ensure_bootstrap_roles(
                    cur,
                    user["id"],
                    email
                ):
